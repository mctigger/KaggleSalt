...
from nets.refine_net_bn_aux import RefineNet, ResNetBase
...
class Model:
    def __init__(self, name, split):
        ...
        self.net = RefineNet(ResNetBase(resnet.resnet50(pretrained=True)), num_features=128)
        self.tta = [
    ...
    def predict(self, net, images):
        ...
        for tta_forward, tta_backward in self.tta:
            masks_predictions, aux = net(tta_forward(images))
    ...
    def train(self, samples_train, samples_val):
        ...
        lr_scheduler_cyclic_1 = utils.CyclicLR(optimizer, 1e-4, 1e-6, 5)
        lr_scheduler_cyclic_2 = utils.CyclicLR(optimizer, 0.1e-4, 0.1e-6, 5)
        lr_scheduler = lr_scheduler_cyclic_1

        criterion = losses.LovaszWithLogitsLoss()
        criterion_segmentation = losses.BCELoss()
        ...
        for e in range(epochs):
            if e > 65:
                lr_scheduler = lr_scheduler_cyclic_2
            ...
            with tqdm(total=len(train_dataloader), leave=False) as pbar, torch.enable_grad():
                ...
                for images, masks_targets in train_dataloader:
                    ...
                    segmentation_loss = criterion(F.adaptive_avg_pool2d(masks_predictions, (101, 101)),
                                     F.adaptive_avg_pool2d(masks_targets, (101, 101)))

                    aux_target = masks_targets.view(masks_targets.size(0), -1)
                    aux_target = torch.sum(aux_target, dim=1) > 0
                    aux_loss = criterion_aux(torch.squeeze(aux), aux_target.float())
                    segmentation = F.sigmoid(masks_predictions) * F.sigmoid(aux).view(-1, 1, 1, 1).expand_as(masks_predictions)
                    segmentation_loss = segmentation_loss + criterion_segmentation(segmentation, masks_targets)

                    loss = segmentation_loss + aux_loss
                    loss.backward()
                    optimizer.step()
                    optimizer.zero_grad()

                    average_meter_train.add('loss', loss.item())
                    self.update_pbar(segmentation, masks_targets, pbar, average_meter_train, 'Training epoch {}'.format(e))
            ...
            with tqdm(total=len(val_dataloader), leave=True) as pbar, torch.no_grad():
                ...
                for images, masks_targets in val_dataloader:
                    ...
                    aux_target = torch.sum(aux_target, dim=1) > 0
                    aux_loss = criterion_aux(torch.squeeze(aux), aux_target.float())
                    segmentation = F.sigmoid(masks_predictions) * F.sigmoid(aux).view(-1, 1, 1, 1).expand_as(
                        masks_predictions)
                    segmentation_loss = segmentation_loss + criterion_segmentation(segmentation, masks_targets)
