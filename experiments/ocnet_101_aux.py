...
from nets.ocnet import get_resnet101_asp_oc_aux
...
class Model:
    def __init__(self, name, split):
        ...
        self.net = get_resnet101_asp_oc_aux()
    ...
    def train(self, net, samples, optimizer, e):
        ...
        with tqdm(total=len(dataloader), leave=False) as pbar, torch.enable_grad():
            ...
            for images, masks_targets in dataloader:
                ...
                loss = criterion(masks_predictions, masks_targets) + 0.4 * criterion(F.upsample(aux, scale_factor=8, mode='bilinear'), masks_targets)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
