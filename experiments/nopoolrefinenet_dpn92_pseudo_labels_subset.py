...
class Model:
    ...
    def fit(self, samples_train, samples_val, samples_test):
        ...
        for e in range(epochs):
            ...
            stats_train = self.train(net, samples_train, samples_test, optimizer, e)
    ...
    def train(self, net, samples, samples_test, optimizer, e):
...
def main():
    ...
    for i, ((samples_train, samples_val), samples_test) in enumerate(zip(utils.mask_stratified_k_fold(), utils.test_k_fold())):
        print("Running split {}".format(i))
        model = Model(name, i)
        stats = model.fit(samples_train, samples_val, samples_test)
