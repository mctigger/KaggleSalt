...
from nets.refinenet_semisupervised import SemisupervisedRefineNet, RefineNetUpsampleClassifier
...
class Model:
    def __init__(self, name, split):
        ...
        self.net = SemisupervisedRefineNet(
            NoPoolResNetBase(resnet34(pretrained=True)),
            num_features=128,
            block_multiplier=1,
    ...
    def train(self, net, samples, optimizer, e):
        ...
        pseudo_dataset = datasets.SemiSupervisedImageDataset(
            ...
            momentum=0.0,
            semisupervised_marker=True
        )

        dataset = datasets.ImageDataset(samples, './data/train', transforms, semisupervised_marker=True)
        ...
        with tqdm(total=len(dataloader), leave=False) as pbar, torch.enable_grad():
            ...
            for images, masks_targets, unsupervised in dataloader:
                masks_targets = masks_targets.to(gpu)
                images = images.to(gpu)
                masks_predictions, reconstruction = net(images)

                loss_segmentation = criterion(masks_predictions[unsupervised == 0], masks_targets[unsupervised == 0])
                loss_reconstruction = criterion(reconstruction[unsupervised], images[unsupervised])
                loss = loss_segmentation + loss_reconstruction

                loss.backward()
                optimizer.step()
                optimizer.zero_grad()

                average_meter_train.add('loss_segmentation', loss_segmentation.item())
                average_meter_train.add('loss_reconstruction', loss_reconstruction.item())
