...
import torch
from torch.nn import DataParallel, functional as F, BCEWithLogitsLoss
...
from nets.refinenet_hypercolumn import AuxHypercolumnCatRefineNet
from nets.refinenet import ModifiedRefineNetUpsampleClassifier
...
class Model:
    def __init__(self, name, split):
        ...
        self.net = AuxHypercolumnCatRefineNet(
            NoPoolDPN92Base(dpn92()),
            num_features=128,
            block_multiplier=1,
            num_features_base=[256 + 80, 512 + 192, 1024 + 528, 2048 + 640],
            classifier=lambda c: ModifiedRefineNetUpsampleClassifier(640, 320, scale_factor=2),
        )
        self.tta = [
    ...
    def train(self, net, samples, optimizer, e):
        ...
        criterion_segmentation = losses.ELULovaszFocalWithLogitsLoss(alpha, 2 - alpha, size_average=False)
        ...
        with tqdm(total=len(dataloader), leave=False) as pbar, torch.enable_grad():
            ...
            for images, masks_targets in dataloader:
                ...
                masks_predictions, aux_segmentation, aux_cls = net(images)

                t_cls = F.adaptive_max_pool2d(masks_targets, 1).view(masks_targets.size(0), -1)
                loss_fuse = criterion(masks_predictions, masks_targets)
                loss_segmentation = torch.mean(0.1 * t_cls * criterion_segmentation(aux_segmentation, masks_targets))
                loss_cls = 0.05 * criterion_cls(aux_cls, t_cls)

                loss = loss_fuse + loss_segmentation + loss_cls
