...
class Model:
    ...
    def train(self, net, samples, optimizer, e):
        ...
        dataset_aux = datasets.BoundaryImageDataset(samples_aux, settings.train, transforms)
        ...
        dataset = datasets.BoundaryImageDataset(samples, settings.train, transforms)
        ...
        with tqdm(total=len(dataloader), leave=False, ascii=True) as pbar, torch.enable_grad():
            ...
            for images, masks_targets, boundary_targets in dataloader:
                ...
                loss = 0.1 * criterion(masks_predictions, masks_targets) + criterion_boundary(boundary_predictions, boundary_targets)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
