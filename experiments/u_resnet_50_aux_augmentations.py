...
import torch
from torch.nn import DataParallel, BCEWithLogitsLoss, MSELoss
...
from nets.u_resnet import AuxBottleneckUResNet
...
class Model:
    def __init__(self, name, split):
        ...
        self.net = AuxBottleneckUResNet(resnet.resnet50(pretrained=True), layers=[3, 4, 6, 3])
        ...
        for e in range(epochs):
            ...
            with tqdm(total=len(train_dataloader), leave=False) as pbar, torch.enable_grad():
                ...
                for images, masks_targets in train_dataloader:
                    ...
                    aux_loss = aux_criterion(aux, (torch.sum(torch.sum(masks_targets, dim=2), dim=2) / 10200))
                    loss = aux_loss + criterion(F.adaptive_avg_pool2d(masks_predictions, (101, 101)),
